...
class CLPageRank : public CLApp {
 ...
 public:
  CLPageRank(int argc, char** argv, std::string name, int s_tile_sz,
             int d_tile_sz) :
    CLApp(argc, argv, name), s_tile_sz_(s_tile_sz), d_tile_sz_(d_tile_sz) {
    get_args_ += "x:y:";
    AddHelpLine('y', "y", "Tile size in the src dimension",
                std::to_string(s_tile_sz_));
    AddHelpLine('x', "x", "Tile size in the dst dimension",
                std::to_string(s_tile_sz_));
  }

  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      case 'x': d_tile_sz_ = atoi(opt_arg);          break;
      case 'y': s_tile_sz_ = atoi(opt_arg);          break;
      default: CLApp::HandleArg(opt, opt_arg);
    }
  }

  int s_tile_sz() const { return s_tile_sz_; }
  int d_tile_sz() const { return d_tile_sz_; }
};
