...
class CLPageRank : public CLApp {
 ...
 public:
  CLPageRank(int argc, char** argv, std::string name, int tile_sz,
             int aspect_ratio) :
    CLApp(argc, argv, name), tile_sz_(tile_sz), aspect_ratio_(aspect_ratio) {
    get_args_ += "x:w:";
    AddHelpLine('x', "x", "Tile size (in baseline square 2d tiling)",
                std::to_string(tile_sz_));
    AddHelpLine('w', "w", "Aspect ratio (for hub vertices)",
                std::to_string(aspect_ratio_));
  }

  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      case 'x': tile_sz_      = atoi(opt_arg);          break;
      case 'w': aspect_ratio_ = atoi(opt_arg);          break;
      default: CLApp::HandleArg(opt, opt_arg);
    }
  }

  int tile_sz() const { return tile_sz_; }
  int aspect_ratio() const { return aspect_ratio_; }
};
