...
class CSRGraph {
 ...
 public:
  ...
  uint64_t out_neighs_in_range(NodeID_ n, NodeID_ startID, NodeID_ endID) const {
    auto it_start = std::lower_bound(out_index_[n], out_index_[n+1], startID); //should include startID
    auto it_end   = std::upper_bound(out_index_[n], out_index_[n+1], endID-1); //should not include endID
    std::vector<NodeID_> rangeNeighs;
    ...
    else
    {
      ...
      {
        auto vtx = *it;
        assert(vtx >= startID && vtx < endID);
        rangeNeighs.push_back(*it);
        ++it;
      }
      return rangeNeighs; 
    }
  ...
  std::vector<NodeID_> in_neighs_in_range(NodeID_ n, NodeID_ startID, NodeID_ endID) const {
    static_assert(MakeInverse, "Graph inversion disabled but reading inverse");
    auto it_start = std::lower_bound(in_index_[n], in_index_[n+1], startID); //should include startID
    auto it_end   = std::upper_bound(in_index_[n], in_index_[n+1], endID-1); //should not include endID
    std::vector<NodeID_> rangeNeighs;
